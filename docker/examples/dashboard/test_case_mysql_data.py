...
def user_build(names: List, country: str, grander: str = "Male") -> List:
    ...
    for i in range(1, len(names) + 1):
        ...
        country = country
        city = secrets.choice(cities[country])
...
def gnerate_all_orders(users, cursor):
    ...
    for i in range(1, orders_num + 1):
        ...
        product_category = secrets.choice(categories)
        product_name = secrets.choice(categories_product[product_category])
        amount = round(secrets.SystemRandom().uniform(0, 10000), 2)
        pay_status = secrets.choice(["SUCCESS", "FAILD", "CANCEL", "REFUND"])
        user_id = secrets.choice(users)[0]
        user_name = secrets.choice(users)[1]
